    ...
    return rx.vstack(
        ...
        rx.box(
            rx.markdown(
                msg.content,
                component_map={
                    "p": lambda text: rx.text(text, size="2", color=styles.TEXT_PRIMARY, class_name="chat-markdown-p"),
                    "li": lambda text: rx.list.item(text, color=styles.TEXT_PRIMARY),
                    "strong": lambda text: rx.text(text, weight="bold", color=styles.TEXT_PRIMARY, as_="span"),
                    "a": lambda text, **props: rx.link(text, color=styles.TEAL_600, **props),
